...
class தரவுத்தளங்கள் {
  ...
  async _மொழி_தத_ஏற்று(திட்டம், வேண்டியமொழி) {
    ...
    const முகவரி = await திட்டம்_தத.get(வேண்டியமொழி)
    var தத

    if (முகவரி) {
      தத = await this.கோள்பாதை.kvstore(முகவரி)
    } else {
      தத = await this.கோள்பாதை.kvstore(வேண்டியமொழி, this.தனிப்பட்டவமைப்புகள)
  ...
  async _பரிந்துரை_தத_ஏற்று(திட்டம், வேண்டியமொழி) {
    ...
    if (முகவரி) {
      ...
      தத = await this.கோள்பாதை.kvstore('பரிந்துரைகள்', this.தனிப்பட்டவமைப்புகள)
  ...
  async பரிந்துரையு(சாபி, உரை, மூலுரை, மூல்மொழி, வேண்டியமொழி, திட்டம்) {
    ...
    console.log(தத)
    தத.set(பரிந்ருரை.குறி, பரிந்ருரை)
    console.log(தத.all)
  }
