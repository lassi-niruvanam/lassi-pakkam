...
export default class லஸ்ஸிபக்கம்_தரவுத்தளங்கள் extends தரவுத்தளங்கள் {
  ...
  async பயனாளர்_பெயரை_பெறு(மொழி, பயனாளர்) {
    பயனாளர் = பயனாளர் || this.பயனாளர்_அடையாளம்
    if (பயனாளர் === this.பயனாளர்_அடையாளம்) {
      const பெயர்_தரவுத்தளம் = await this.தத.தத_பெறு('பயனாளர்பெயர்கள்')
      return பெயர்_தரவுத்தளம்.get(மொழி)
    }
    return undefined
  }
  ...
  async பயனாளர்_படத்தை_பெறு(பயனாளர்) {
    பயனாளர் = பயனாளர் || this.பயனாளர்_அடையாளம்
    if (பயனாளர் === this.பயனாளர்_அடையாளம்) {
  ...
  async மொழிபெயர்ப்பை_பரிந்துரையு(
    சாபி, பரிந்துரை, மூலுரை, மூல்மொழி, இலக்குமொழி, திட்டம்
  ) {
    மூலுரை = MD5(மூலுரை).toString()
    const பரிந்ருரை = {
      குறி: uuidv4(), சாபி, பரிந்துரை, மூலுரை, மூல்மொழி, தேதி: Date.now()
    }
    await this.தத.மதிப்பை_வையு(
      ['மொழியாக்கம்', திட்டம், இலக்குமொழி, 'மொழிபெயர்ப்புகள்'],
      பரிந்ருரை.குறி, பரிந்ருரை
    )
  }

  async பரிந்துரையைநீக்கு(குறி, இலக்குமொழி, திட்டம்){
    const தத = await this.தத.தத_பெறு(['மொழியாக்கம்', திட்டம், இலக்குமொழி, 'மொழிபெயர்ப்புகள்'])
    await தத.del(குறி)
  }

  async பரிந்துரைகளை_பெறு(சாபி, இலக்குமொழி, திட்டம்){
    const தத = await this.தத.தத_பெறு(['மொழியாக்கம்', திட்டம், இலக்குமொழி, 'மொழிபெயர்ப்புகள்'])
    console.log('ici', தத.id)
    const பரிந்துரைகள் = Object.values(தத.all).map(
      (ப)=>{return {...ப, பங்களிப்பாளர்: this.பயனாளர்_அடையாளம்}}
    ).filter(இ => இ.சாபி===சாபி)
    console.log(திட்டம், சாபி, இலக்குமொழி, பரிந்துரைகள்)
    return பரிந்துரைகள்
  }
...
class தத_மரம் extends EventEmitter {
  ...
  async தத_பெறு(பெயர்) {
    ...
    async function _தத_பெறு(பாதை, மூல்தத, மூல்தத_பாதை = []) {
      ...
      console.log(தன்._ததளங்கள், சாபி)
      ...
      console.log({சாபி, தத, பாதை})
      தன்._ததளங்கள்[தத_பெயர்] = தத
      for (let இ of ['write', 'replicated', 'ready']) {
